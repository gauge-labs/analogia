...
export class PagesManager {
  ...
  private getActiveWebview(): Electron.WebviewTag | undefined {
    return (
      this.editorEngine.webviews.selected[0] ||
      this.editorEngine.webviews.getAll()[0]
    );
  }
  ...
  public isNodeActive(node: PageNode): boolean {
    ...
    return nodeSegments.every((nodeSegment, index) => {
      ...
      const isDynamic =
        nodeSegment.startsWith("[") && nodeSegment.endsWith("]");
      ...
      return (
        nodeSegment.replace(/\\/g, "") === activeSegment.replace(/\\/g, "")
      );
    });
  }
