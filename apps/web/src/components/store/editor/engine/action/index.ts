...
import { assertNever } from "@analogia/utility";
...
export class ActionManager {
  ...
  private insertElement({
    ...
    targets.forEach((elementMetadata) => {
      const webview = this.editorEngine.webviews.getWebview(
        elementMetadata.webviewId,
      );
      if (!webview) {
        console.error("Failed to get webview");
        return;
  ...
  private groupElements({ parent, container, children }: GroupElementsAction) {
    ...
    sendToWebview(webview, WebviewChannels.GROUP_ELEMENTS, {
      parent,
      container,
      children,
    });
  ...
  private ungroupElements({
    ...
    sendToWebview(webview, WebviewChannels.UNGROUP_ELEMENTS, {
      parent,
      container,
      children,
    });
