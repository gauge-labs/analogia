...
import type {
  ...
  ProjectSettings,
  RectPosition,
} from "@analogia/models/projects";
import { Icons, type IconProps } from "@analogia/ui-v4/icons/index";
...
export class CanvasManager {
  ...
  async applySettings(project: Project) {
    ...
    this.panPosition =
      project.settings?.position || this.getDefaultPanPosition();
    ...
    if (project.settings?.frames && project.settings.frames.length) {
      ...
      const desktopPreset = SIZE_PRESETS.find(
        (preset) => preset.name === "Desktop",
      );
      const mobilePreset = SIZE_PRESETS.find(
        (preset) => preset.name === "Mobile",
      );
  ...
  getDefaultFrame(defaults: Partial<FrameSettings>): FrameSettings {
    return {
      ...
      aspectRatioLocked:
        defaults.aspectRatioLocked || DefaultSettings.ASPECT_RATIO_LOCKED,
