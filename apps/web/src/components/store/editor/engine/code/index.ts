...
export class CodeManager {
  ...
  async getFileContent(
    ...
    return invokeMainChannel(MainChannels.GET_FILE_CONTENT, {
      filePath,
      stripIds,
    });
  }
  ...
  async writeInsert({
    ...
    const oidToCodeChange = new Map<string, CodeDiffRequest>();
    const insertedEl = getInsertedElement(
      element,
      location,
      pasteParams,
      codeBlock,
  ...
  private async writeCode(action: WriteCodeAction) {
    const res = await invokeMainChannel(
      MainChannels.WRITE_CODE_DIFFS,
      action.diffs,
    );
    if (!res) {
  ...
  async getAndWriteCodeDiff(
    ...
    if (useHistory) {
      ...
      // Write code directly
      codeDiffs = await invokeMainChannel(
