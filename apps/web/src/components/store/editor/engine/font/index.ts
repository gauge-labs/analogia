...
import { fontFamilies, MainChannels } from "@analogia/models";
...
export class FontManager {
  ...
  constructor(
    ...
    const disposer = reaction(
      ...
      (folderPath) => {
        console.log(
          "Project path changed, setting up font watcher:",
          folderPath,
        );
        if (folderPath) {
  ...
  private async watchFontFile(projectRoot: string) {
    if (!projectRoot) {
      console.error(
        "No project root provided, skipping font file watcher setup",
      );
      return;
    }
  ...
  async getDefaultFont() {
    ...
    try {
      const defaultFont = await invokeMainChannel(
        MainChannels.GET_DEFAULT_FONT,
        {
          projectRoot,
        },
      );
