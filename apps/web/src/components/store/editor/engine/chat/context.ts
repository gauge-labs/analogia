...
export class ChatContext {
  ...
  async getChatContext(): Promise<ChatMessageContext[]> {
    ...
    if (selected.length) {
      highlightedContext = await this.getHighlightedContext(
        selected,
        fileNames,
      );
    }
  ...
  private async getFileContext(
    ...
    for (const fileName of fileNames) {
      const fileContent = await this.editorEngine.code.getFileContent(
        fileName,
        false,
      );
      if (fileContent === null) {
  ...
  async getScreenshotContext(): Promise<ImageMessageContext | null> {
    ...
    try {
      const result = await this.editorEngine.takeWebviewScreenshot(
        screenshotName,
        webviewId,
      );
