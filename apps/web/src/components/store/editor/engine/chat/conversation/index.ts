...
export class ConversationManager {
  ...
  async generateConversationSummary(): Promise<void> {
    ...
    const res: string | null = await invokeMainChannel(
      MainChannels.GENERATE_CHAT_SUMMARY,
      {
  ...
  addUserMessage(
    ...
    const newMessage = UserChatMessageImpl.fromStringContent(
      stringContent,
      context,
    );
  ...
  addCoreToolMessage(
    coreMessage: CoreToolMessage,
  ): ToolChatMessageImpl | undefined {
    if (!this.current) {
      console.error("No conversation found");
      return;
    }
