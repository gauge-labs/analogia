...
export class GroupManager {
  ...
  async groupSelectedElements() {
    ...
    const groupAction = await this.getGroupAction(
      webviewId,
      parentDomId,
      selectedEls,
    );
  ...
  canGroupElements() {
    return (
      this.getGroupParentId(this.editorEngine.elements.selected, false) !== null
    );
  }
  ...
  async getUngroupAction(
    selectedEl: DomElement,
  ): Promise<UngroupElementsAction | null> {
