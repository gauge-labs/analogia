...
import { DEFAULT_COLOR_NAME, MainChannels } from "@analogia/models";
import type {
  ConfigResult,
  ParsedColors,
  ThemeColors,
...
export class ThemeManager {
  ...
  async scanConfig() {
    ...
    try {
      const configResult = (await invokeMainChannel(
      ...
      const ungroupedKeys = Object.keys(parsed).filter((key) => {
        ...
        const isPrefix = Object.values(groups).some((set) =>
          Array.from(set).some((groupedKey) =>
            groupedKey.startsWith(key + "-"),
          ),
        );
  ...
  generateDefaultColors(
    ...
    Object.keys(colors)
      ...
      .forEach((colorName) => {
        ...
        if (
          typeof defaultColorScale !== "object" ||
        ...
        const colorItems: ColorItem[] = Object.entries(defaultColorScale)
          ...
          .map(([shade, defaultValue]) => {
            ...
            const darkModeValue =
              darkModeColors[`${colorName}-${shade}`]?.value;
  ...
  async update(
    ...
    try {
      ...
      const originalKey =
        this.brandColors[originalGroupName]?.[index]?.originalKey || "";
  ...
  async duplicate(
    ...
    try {
      if (isDefaultPalette) {
        ...
        await this.handleDefaultColorChange(
          groupName,
          colorIndex,
          color,
          theme,
        ...
        const colorToDuplicate = group?.find(
  ...
  getColorByName(colorName: string): string | undefined {
    ...
    if (brandGroup) {
      if (!shadeName || shadeName === DEFAULT_COLOR_NAME) {
        const defaultColor = brandGroup.find(
          (color) => color.name === DEFAULT_COLOR_NAME,
        );
