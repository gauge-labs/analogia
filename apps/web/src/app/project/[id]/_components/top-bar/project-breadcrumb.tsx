...
export const ProjectBreadcrumb = () => {
  ...
  return (
    <div className="text-small mx-2 flex flex-row items-center gap-2">
      <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            variant={"ghost"}
            className="text-foreground-analogia text-small hover:text-foreground-active mx-0 gap-2 px-0 hover:bg-transparent"
          >
            <Icons.AnalogiaLogo
              className={cn(
                "hidden h-6 w-6 md:block",
                isClosingProject && "animate-pulse",
              )}
            />
            <span className="text-foreground-analogia text-small mx-0 max-w-[60px] cursor-pointer truncate px-0 md:max-w-[100px] lg:max-w-[200px]">
              {isClosingProject ? "Stopping project..." : project.name}
            </span>
            <Icons.ChevronDown className="text-foreground-analogia rotate-0 transition-all duration-200 ease-in-out group-data-[state=open]:-rotate-180" />
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent
          align="start"
        ...
        >
          <DropdownMenuItem onClick={() => handleNavigateToProjects()}>
            <div className="row center group flex items-center">
              <Icons.Tokens className="mr-2 transition-transform group-hover:rotate-12" />
