...
export const MeasurementOverlay: React.FC<MeasurementProps> = memo(
  ({ fromRect, toRect }) => {
    const editorEngine = useEditorEngine();
    const webview = editorEngine.webview.getWebview(
    ...
    const createDistance = (
      ...
      if (isHorizontal && !isBetween(start.y, toRect.top, toRect.bottom)) {
        ...
        !isHorizontal &&
        !isBetween(start.x, toRect.left, toRect.right)
      ) {
        result.supportLine = {
    ...
    const distances = useMemo(() => {
      ...
      const scaleValue = (value: number) =>
        adaptValueToCanvas(Math.abs(value), true);
      ...
      if (isIntersect(fromRectPoint, toRectPointResult)) {
        ...
        if (insideRect) {
          ...
          y =
            fromRectPoint.top +
            (toRectPointResult.bottom - fromRectPoint.top) / 2;
        } else {
          y =
            fromRectPoint.bottom -
            (fromRectPoint.bottom - toRectPointResult.top) / 2;
        }
        ...
        isBetween(
          fromRectPoint.left,
          toRectPointResult.left,
          toRectPointResult.right,
        ) &&
      ...
      if (isIntersect(fromRectPoint, toRectPointResult)) {
        ...
        if (insideRect) {
          ...
          x =
            fromRectPoint.left +
            (toRectPointResult.right - fromRectPoint.left) / 2;
        } else {
          x =
            fromRectPoint.right -
            (fromRectPoint.right - toRectPointResult.left) / 2;
        }
        ...
        isBetween(
          fromRectPoint.top,
          toRectPointResult.top,
          toRectPointResult.bottom,
        )
      ) {
        const distance: Distance = {
          value: scaleValue(fromRectPoint.top - toRectPointResult.top),
    ...
    const svgContent = (
      <g transform={`translate(${-viewBox.minX},${-viewBox.minY})`}>
        ...
        {distances.map((distance, index) => {
          ...
          const midX =
            (distance.start.x + distance.end.x) / 2 + (isHorizontal ? 24 : 0);
          const midY =
            (distance.start.y + distance.end.y) / 2 + (isHorizontal ? 0 : 16);
