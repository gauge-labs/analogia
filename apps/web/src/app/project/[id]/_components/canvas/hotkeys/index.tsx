...
export const HotkeysArea = ({ children }: { children: ReactNode }) => {
  ...
  useHotkeys("space", () => (editorEngine.state.editorMode = EditorMode.PAN), {
    keydown: true,
  });
  useHotkeys(
    "space",
  ...
  useHotkeys("alt", () => editorEngine.elements.showMeasurement(), {
    keydown: true,
  });
  ...
  useHotkeys(Hotkey.ENTER.command, () =>
  ...
  // Move
  useHotkeys(Hotkey.MOVE_LAYER_UP.command, () =>
    editorEngine.move.moveSelected("up"),
  );
  useHotkeys(Hotkey.MOVE_LAYER_DOWN.command, () =>
